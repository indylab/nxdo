...
def nfsp_leduc_dqn_params(action_space: Space) -> Dict:
    assert isinstance(action_space, Discrete)
    action_space: Discrete = action_space
    return {
        "framework": "torch",
...
def leduc_dqn_params_lowered(action_space: Space) -> Dict:
    return {
        "framework": "torch",
        # Smooth metrics over this many episodes.
        "metrics_smoothing_episodes": 1000,
...
def nfsp_oshi_zumo_dqn_params_like_kuhn(action_space: Space) -> Dict:
    assert isinstance(action_space, Discrete)
    action_space: Discrete = action_space
    return {
        "framework": "torch",
...
def nfsp_80x_dummy_leduc_avg_policy_params_gpu(action_space: Space) -> Dict:
    params = nfsp_leduc_avg_policy_params_gpu(action_space=action_space)
...
def nfsp_80x_dummy_leduc_params_gpu(action_space: Space) -> Dict:
    params = nfsp_leduc_dqn_params_gpu(action_space=action_space)
    params["model"]["custom_model"] = get_valid_action_fcn_class(obs_len=_LEDUC_OBS_LEN, action_space_n=action_space.n, dummy_actions_multiplier=80)
    return params
...
def nfsp_kuhn_sac_params(action_space: Space) -> Dict:
    ...
    raise NotImplementedError("need to set up SAC valid actions")
