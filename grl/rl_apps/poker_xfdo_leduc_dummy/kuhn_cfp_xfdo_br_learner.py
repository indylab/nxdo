...
from grl.rl_apps.kuhn_poker_p2sro.config import kuhn_sac_params, leduc_dqn_params
...
def train_poker_best_response(br_player: int, print_train_results: bool = True):
    ...
    xfdo_manager = RemoteXFDOManagerClient(n_players=2, port=os.getenv("XFDO_PORT", 4545), remote_server_host="127.0.0.1")
    ...
    base_env_config = {'version': "leduc_poker", "fixed_players": True, "append_valid_actions_mask_to_obs": True, "dummy_action_multiplier": 20}
    ...
    trainer_config = merge_dicts(trainer_config, leduc_dqn_params(action_space=tmp_env.base_action_space))
