...
def sync_active_policy_br_and_metanash_with_p2sro_manager(trainer: SACTrainer,
    ...
    p2sro_manager.submit_new_active_policy_metadata(
        ...
        metadata_dict=create_metadata_with_new_checkpoint_for_current_best_response(
            trainer=trainer, save_dir=BR_CHECKPOINT_SAVE_DIR, timesteps_training_br=timesteps_training_br,
            episodes_training_br=episodes_training_br,
            active_policy_num=active_policy_num
        ))
...
def train_poker_sac_best_response(print_train_results=True):
    ...
    trainer_config = merge_dicts(trainer_config, kuhn_sac_params(action_space=tmp_env.action_space))
    ...
    active_policy_spec: PayoffTableStrategySpec = p2sro_manager.claim_new_active_policy_for_player(
        player=0, new_policy_metadata_dict=create_metadata_with_new_checkpoint_for_current_best_response(
            trainer=trainer, save_dir=BR_CHECKPOINT_SAVE_DIR, timesteps_training_br=0, episodes_training_br=0,
            active_policy_num=None
        ))

    log(f"active policy pure strat indexes are: {active_policy_spec._pure_strategy_indexes}")
    ...
    sync_with_manager_every_n_train_iters = 10

    dont_do_saturation_checks_before_n_train_iters = 40
    ...
    p2sro_manager.set_active_policy_as_fixed(
        ...
        final_metadata_dict=create_metadata_with_new_checkpoint_for_current_best_response(
            trainer=trainer, save_dir=BR_CHECKPOINT_SAVE_DIR, timesteps_training_br=total_timesteps_training_br,
            episodes_training_br=total_episodes_training_br,
            active_policy_num=active_policy_num
        ))
...
if __name__ == "__main__":
    ...
    train_poker_sac_best_response(print_train_results=True)
    train_poker_sac_best_response(print_train_results=True)
