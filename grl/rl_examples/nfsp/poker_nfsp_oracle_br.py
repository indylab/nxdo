...
from grl.rl_examples.nfsp.openspiel_utils import nfsp_measure_exploitability_nonlstm

from grl.rl_examples.nfsp.poker_oracle_br_policy import PokerOracleBestResponsePolicy
...
def train_poker_off_policy_rl_nfsp(results_dir: str,
    ...
    class NFSPBestResponseCallbacks(DefaultCallbacks):
        ...
        def on_sample_end(self, *, worker: "RolloutWorker", samples: SampleBatch, **kwargs):
            ...
            for average_policy_id, br_policy_id in [("average_policy_0", "best_response_0"), ("average_policy_1", "best_response_1"), ("average_policy_1", "br_q_stub")]:
                for policy_id, policy_samples in samples.policy_batches.items():
                    if policy_id == br_policy_id:
                        assert policy_id in ["best_response_0", "best_response_1"]
        ...
        def on_train_result(self, *, trainer, result: dict, **kwargs):
            ...
            # result["avg_trainer"] = trainer.latest_avg_trainer_result
    ...
    br_trainer_config = {
        ...
        "multiagent": {
            ...
            "policies": {
                ...
                "best_response_0": (PokerOracleBestResponsePolicy, tmp_env.observation_space, tmp_env.action_space, {}),
                ...
                "best_response_1": (PokerOracleBestResponsePolicy, tmp_env.observation_space, tmp_env.action_space, {}),
                "br_q_stub": (br_policy_class, tmp_env.observation_space, tmp_env.action_space, {}),
            },
        ...
        "explore": True, # TODO does this break things?
    }
    ...
    while True:
        ...
        # set best responses
        local_br_0: PokerOracleBestResponsePolicy = br_trainer.workers.local_worker().policy_map["best_response_0"]
        local_br_1: PokerOracleBestResponsePolicy = br_trainer.workers.local_worker().policy_map["best_response_1"]
        local_avg_policy_0: NFSPTorchAveragePolicy = br_trainer.workers.local_worker().policy_map["average_policy_0"]
        local_avg_policy_1: NFSPTorchAveragePolicy = br_trainer.workers.local_worker().policy_map["average_policy_1"]

        new_local_br_0_dict = local_br_0.compute_best_response(policy_to_exploit=local_avg_policy_1, br_only_as_player_id=0)
        new_local_br_1_dict = local_br_1.compute_best_response(policy_to_exploit=local_avg_policy_0, br_only_as_player_id=1)

        def _one_off_set_worker_br_dicts(worker: RolloutWorker):
            br_0: PokerOracleBestResponsePolicy = worker.policy_map["best_response_0"]
            br_1: PokerOracleBestResponsePolicy = worker.policy_map["best_response_1"]
            br_0.set_policy_dict(policy_dict=new_local_br_0_dict)
            br_1.set_policy_dict(policy_dict=new_local_br_1_dict)
        br_trainer.workers.foreach_worker(_one_off_set_worker_br_dicts)

        br_trainer.latest_avg_trainer_result = avg_train_results
        print("br train...")
