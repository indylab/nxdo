...
def run_poker_evaluation_loop(commandline_args,
    ...
    if commandline_args.env.lower() == 'simple_push':
        env = SimplePushMultiAgentEnv(env_config={})
    else:
        raise NotImplementedError(f"Unknown env arg: {commandline_args.env}")
    ...
    policies = [policy_class(env.observation_space,
                             env.action_space,
                             merge_dicts(DEFAULT_SAC_CONFIG, simple_push_sac_params(action_space=env.action_space)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='simple_push', help="[simple_push]")
    parser.add_argument('--algo', type=str, default='sac', help="[sac]")

    commandline_args = parser.parse_args()

    num_workers = 8
    ray.init(ignore_reinit_error=False, local_mode=False, num_cpus=num_workers)
