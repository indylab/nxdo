...
from grl.rl_examples.sumo_p2sro.config import sumo_ppo_params
...
def checkpoint_dir(trainer: PPOTrainer):
    return os.path.join(trainer.logdir, "br_checkpoints")


def save_best_response_checkpoint(trainer: PPOTrainer,
                                  save_dir: str,
...
def load_metanash_pure_strat(policy: PPOTorchPolicy, pure_strat_spec: PayoffTableStrategySpec):
...
def create_metadata_with_new_checkpoint_for_current_best_response(trainer: PPOTrainer,
                                                              save_dir: str,
                                                              timesteps_training_br: int,
                                                              episodes_training_br: int,
                                                              active_policy_num: int = None,
...
def set_best_response_active_policy_spec_for_all_workers(trainer: PPOTrainer,
                                                         active_policy_spec: PayoffTableStrategySpec):
    def _set_p2sro_policy_spec_on_best_response_policy(worker: RolloutWorker):
        br_policy: PPOTorchPolicy = worker.policy_map["best_response"]
...
def update_all_workers_to_latest_metanash(trainer: PPOTrainer,
...
def sync_active_policy_br_and_metanash_with_p2sro_manager(trainer: PPOTrainer,
                                                            p2sro_manager: RemoteP2SROManagerClient,
                                                            active_policy_num: int,
                                                            timesteps_training_br: int,
                                                            episodes_training_br: int):
...
# def override_best_response_sac_target_entropy(trainer: PPOTrainer, new_target_entropy: float):
...
class P2SROPreAndPostEpisodeCallbacks(DefaultCallbacks):
    ...
    def on_episode_start(self, *, worker: RolloutWorker, base_env: BaseEnv,
        ...
        metanash_policy: PPOTorchPolicy = policies["metanash"]
    ...
    #         policy: PPOTorchPolicy = worker.policy_map[policy_name]
...
def train_poker_sac_best_response(results_dir, print_train_results=True):
    ...
    trainer_config = {
        ...
        "num_gpus": 0.6,
        "num_workers": 25,
        "num_gpus_per_worker": 1.4/25,
        "num_envs_per_worker": 2,
        "multiagent": {
            "policies_to_train": ["best_response"],
            "policies": {
                "metanash": (PPOTorchPolicy, tmp_env.observation_space, tmp_env.action_space, {}),
                "best_response": (PPOTorchPolicy, tmp_env.observation_space, tmp_env.action_space, {}),
            },
            "policy_mapping_fn": select_policy,
        },
    }
    trainer_config = merge_dicts(trainer_config, sumo_ppo_params(action_space=tmp_env.action_space))
