...
from grl.eval_dispatcher import EvalDispatcherWithServer, EvalResult
from grl.p2sro_manager.logger import P2SROManagerLogger, SimpleP2SROManagerLogger
...
class _P2SROPlayerStats(object):
    ...
    def move_active_policy_to_fixed(self, policy_num):
        if policy_num not in self.active_policy_indexes:
            raise ValueError(f"Policy {policy_num} isn't an active policy. "
...
class P2SROManager(object):
    ...
    def __init__(self,
        ...
        self._is_two_player_symmetric = is_two_player_symmetric
        ...
        self._payoff_table = PayoffTable(n_players=n_players, exponential_average_coeff=None)

        num_player_stats = 1 if self._is_two_player_symmetric else n_players
        ...
        self._eval_dispatcher = EvalDispatcherWithServer(games_per_eval=self._games_per_external_payoff_eval,
                                                         game_is_two_player_symmetric=self._is_two_player_symmetric,
        ...
        if manager_logger is None:
            manager_logger = SimpleP2SROManagerLogger(p2sro_manger=self, log_dir=f"/tmp/p2sro_{datetime_str()}")
    ...
    def claim_new_active_policy_for_player(self, player, new_policy_metadata_dict) -> PayoffTableStrategySpec:
        with self._modification_lock:
            ...
            if self._is_two_player_symmetric and player != 0:
            ...
            if self._is_two_player_symmetric:
    ...
    def can_active_policy_be_set_as_fixed_now(self, player, policy_num) -> bool:
        with self._modification_lock:
            ...
            for other_player, player_stats in enumerate(self._player_stats):
                if other_player == player and not self._is_two_player_symmetric:
                    continue
    ...
    def set_active_policy_as_fixed(self, player, policy_num, final_metadata_dict) -> PayoffTableStrategySpec:
        with self._modification_lock:
            ...
            if self._is_two_player_symmetric and player != 0:
                raise ValueError("Always use player 0 for two player symmetric game (plays both sides).")
            if policy_num not in self._player_stats[player]:
                raise ValueError(f"Policy {policy_num} isn't an active policy for player {player}.")
            ...
            if self._do_external_payoff_evals_for_new_fixed_policies:
                ...
                # Track which eval matchups we're waiting on.
                self._pending_spec_matchups_for_new_fixed_policies[(player, policy_num)] = set(fixed_policy_spec_matchups)
            else:
    ...
    def get_copy_of_latest_data(self) -> (PayoffTable, List[List[int]], List[List[int]]):
        with self._modification_lock:
            # Update our payoff table with any eval results that have come in since the last time data was requested.
            self._process_any_finished_external_evals()
    ...
    def submit_empirical_payoff_result(self,
        ...
        with self._modification_lock:
            ...
            if self._is_two_player_symmetric and payoffs_for_each_player[0] != -payoffs_for_each_player[1]:
            ...
            for player in range(self._n_players):
                self._payoff_table.add_empirical_payoff_result(
                    ...
                    payoff=payoffs_for_each_player[player],
                    games_played=games_played,
                    override_all_previous_results=override_all_previous_results)

            self._manager_logger.on_submitted_empirical_payoff_result(
        ...
        for spec_list_player in range(self._n_players):
            if spec_list_player == as_player:
                ...
                if fixed_policies_only:
                    policy_nums_to_consider = self._player_stats[spec_list_player].fixed_policy_indexes
                else:
                    policy_nums_to_consider = self._player_stats[spec_list_player].active_policy_indexes + \
                                              self._player_stats[spec_list_player].fixed_policy_indexes
    ...
    def _process_any_finished_external_evals(self):
        # Update our payoff table with any payoff results that have come in since the last time we checked.

        finished_evals: List[EvalResult] = self._eval_dispatcher.get_unclaimed_eval_results()
        for eval_result in finished_evals:
            ...
            for player, new_fixed_policy_num in self._pending_spec_matchups_for_new_fixed_policies.keys():
                matchups_for_fixed_policy = self._pending_spec_matchups_for_new_fixed_policies[new_fixed_policy_num]
                if eval_result.policy_specs_for_each_player in matchups_for_fixed_policy:
                    ...
                    if len(matchups_for_fixed_policy) == 0:
                        ...
                        del self._pending_spec_matchups_for_new_fixed_policies[new_fixed_policy_num]
                        # Move it to fixed.
            ...
            self._manager_logger.on_external_eval_result(
                policy_specs_for_each_player=eval_result.policy_specs_for_each_player,
                payoffs_for_each_player=eval_result.payoff_for_each_player,
                games_played=eval_result.games_played,
                overrode_all_previous_results=eval_result_should_override_previous_results
            )