...
from ray.rllib.execution.rollout_ops import ParallelRollouts, StandardizeFields, SelectExperiences
from ray.rllib.execution.train_ops import TrainOneStep, TrainTFMultiGPU
from ray.rllib.policy.policy import Policy
from ray.rllib.utils.typing import TrainerConfigDict
from ray.util.iter import LocalIterator

from grl.rllib_tools.concat_batches_single_agent import ConcatBatchesForSingleAgent
...
def validate_config(config: TrainerConfigDict) -> None:
    ...
    if config["multiagent"]["policies_to_train"] and len(config["multiagent"]["policies_to_train"]) != 1:
        raise ValueError("PPOTrainerMultiAgentBatchCounting can only train a single policy.\n"
...
def execution_plan(workers: WorkerSet,
    ...
    rollouts = rollouts.combine(
        ConcatBatchesForSingleAgent(min_batch_size=config["train_batch_size"],
